...
resourcestring
  ...
  dxSBAR_IMAGEINDEX = 'Índice de imagem';
  ...
  dxSBAR_WANTTORESETTOOLBAR = 'Tem certeza de que quer redefinir as alterações feitas no ''%s'' toolbar?';
  dxSBAR_WANTTORESETUSAGEDATA = 'Isso irá excluir o registro dos comandos que você usa neste aplicativo e restaurará o conjunto padrão de comandos visíveis aos menus e barras de ferramentas. Não vai desfazer as personalizações explícitas. Tem certeza de que quer prosseguir?';
  ...
  dxSBAR_NOBARMANAGERS = 'Não há TdxBarManagers acessível';
  ...
  dxSBAR_RECURSIVEMENUS = 'Você não pode criar menus recursivos';
  dxSBAR_COMMANDNAMECANNOTBEBLANK = 'Um nome de comando não pode estar em branco. Por favor, insira um nome.';
  dxSBAR_TOOLBAREXISTS = 'Uma toolbar nomeada ''%s'' já existe. Digite outro nome.';
  dxSBAR_RECURSIVEGROUPS = 'You cannot create recursive groups';
  dxSBAR_WANTTODELETECOMPLEXITEM = 'Um dos objetos selecionados é um item que tem vários links. Tem certeza de que deseja excluir esses links?';
  dxSBAR_CANTPLACEQUICKACCESSGROUPBUTTON = 'Você pode colocar TdxRibbonQuickAccessGroupButton apenas em TdxRibbonQuickAccessToolbar';
  dxSBAR_QUICKACCESSGROUPBUTTONTOOLBARNOTDOCKEDINRIBBON = 'Quick Access Group Button''s Toolbar is not docked in the Ribbon';
  dxSBAR_QUICKACCESSALREADYHASGROUPBUTTON = 'A barra de ferramentas Acesso rápido já contém GroupButton com a mesma barra de ferramentas';
  ...
  dxSBAR_NEWBUTTONCAPTION = 'Botão novo';
  ...
  dxSBAR_CP_ADDBUTTON = 'Adic &Botão';
